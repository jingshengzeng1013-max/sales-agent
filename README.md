...
| 层级 | 技术 |
|------|------|
| 数据采集 | Python 爬虫 + 代理池 + Smart Web Fetch 降级抓取 |
...
# 可选：Smart Web Fetch 技术栈抓取 CCGP 首页链接
python src/crawler/ccgp_smart_fetch.py --max-items 50 --print-items

# 可选：Smart Web Fetch 搜“采购公告/搜全文/通信”并抓详情
python src/crawler/ccgp_smart_search.py --keyword 通信 --pages 1 --crawl-details --detail-limit 3 --print-items

# 或直接使用 CCGP 搜索页 URL
python src/crawler/ccgp_smart_search.py --search-url "https://search.ccgp.gov.cn/bxsearch?searchtype=2&page_index=1&start_time=&end_time=&timeType=2&searchparam=&searchchannel=0&dbselect=bidx&kw=%E9%80%9A%E4%BF%A1&bidSort=0&pinMu=0&bidType=0&buyerName=&projectId=&displayZone=&zoneId=&agentName=" --crawl-details --detail-limit 3 --print-items
...
### Smart Web Fetch 采集栈
新增 `markdown.new → defuddle.md → r.jina.ai → Scrapling → Playwright` 五层降级抓取链，用于传统列表页爬虫失败时的公开网页采集兜底。`get_data/src/crawler/ccgp_smart_fetch.py` 用于首页链接验证，`get_data/src/crawler/ccgp_smart_search.py` 用于中国政府采购网“采购公告 + 搜全文”列表和详情采集。
